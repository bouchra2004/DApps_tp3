...
function ActionForm({ action, contractName, onResult }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      onResult(action.format ? action.format(printable) : printable)
    } catch (err) {
      onResult(`Erreur: ${err.message}`)
    } finally {
      setLoading(false)
    }
  }
...
export default function ExercicePage({ exercise }) {
  ...
  const [result, setResult] = useState('Pret a interagir avec le contrat.')
        ...
        <div className="grid gap-6 p-6 lg:grid-cols-[1fr_360px]">
          <div className="space-y-4">
            {exercise.actions.map((action) => (
              <ActionForm key={`${action.method}-${action.label}`} action={action} contractName={exercise.contract} onResult={setResult} />
            ))}
          </div>
          ...
          <aside className="h-fit rounded-lg border border-slate-200 bg-slate-50 p-5">
            ...
            <div className="mt-3 rounded-md border border-slate-200 bg-white p-4 text-sm font-semibold leading-6 text-slate-800 shadow-sm">
              {result}
            </div>
          </aside>
        </div>
